fetch('./file.json') // Replace with the actual URL of your hosted JSON file
    ...
    .then(data => {
        ...
        init.children[2].addEventListener('click', ()=>{
            if(init.children[0].value > 0 && init.children[0].value <= 400) minimum = parseInt(init.children[0].value)
            if(init.children[1].value > 0 && init.children[1].value <= 400) maximum = parseInt(init.children[1].value)
            if(minimum == 0 || maximum == 0 || maximum - minimum + 1 < 4){
                init.children[3].innerText = "開始請大於0，結束請小於400，題目至少四題，請重輸"
            }
            else{
